...
def _(mo):
    ...
    precio_combustible_number = mo.ui.number(label="Precio del Combustible ", value = 899)
...
def _(beneficios_sociales, dataclass, hrpl_a, hrpl_m, precio_combustible):
    ...
    class Maquina:
        ...
        def costos_mantencion_reparacion(self) -> float:
            ...
            ) / self.horas_productivas_anuales
...
def _(Volteo, pd):
    ...
    return VOLTEO_A, VOLTEO_B, VOLTEO_C


@app.cell
def _(VOLTEO_A, VOLTEO_B, VOLTEO_C):
    VOLTEO_A.cvm_hrpr, VOLTEO_B.cvm_hrpr, VOLTEO_C.cvm_hrpr, (VOLTEO_A.cvm_hrdi / VOLTEO_A.disponibilidad) 
    return
...
def _(PT, Transporte, pd):
    ...
    TRANSPORTE_A = Transporte(
        ...
        disponibilidad=1,
        factor_reventa=0.1,
        factor_reparacion=0.1,
        interes = 0.12, # decimal
        impuestos = 0.175, # decimal
        seguros = 0.02, # decimal
        uso_anual_planificado = 3000,
        consumo_combustible=1,
        factor_lubricantes=0.106,
        sueldo_operador=1,
    )
...
def _(mo):
    ...
    La *productividad de los subsistemas* es la razón entre la producción respecto del tiempo requerido para conseguirla. La unidad más común para expresarla es el volumen respecto del tiempo $(m^3hr^{-1})$. El tiempo se puede clasificar en 3 categorías: *tiempo planificado*, correspondiente a la jornada laboral establecida para una máquina o subsitema; *tiempo disponible* que es el tiempo en que una máquina está en condiciones de ejecutar trabajos (es decir descontando el tiempo planificado de mantención y carga de combustible); *tiempo productivo*, que es el tiempo en que una máquina esta efectivamente realizando el trabajo para el cual fué diseñado. La disponibilidad a nivel de subsistema se calcula considerando la productividad individual de cada máquina, su disponibilidad mecánica y la cantidad de la máquina seleccionada, de la siguiente manera:
    - Sean:
      - $PMhrpr_{i} =$ Productividad individual de la máquina seleccionada en el subsistema $i$, en horas **productivas** $(m^3hrpr^{-1})$.
      - $DM_i =$ Disponibilidad mecánica de la máquina seleccionada en el subsistema $i$ $(\%)$.
      - $PMhrdi_i$ = Productividad individual de la máquina seleccionada en el subsistema $i$, en horas **disponibles** $(m^3hrdi^{-1})$, donde:
       - \[PMhrdi_i = PMhrpr_i \cdot DM_i\]
      - $N_i =$ Cantidad de la máquina seleccionada en el subsistema $i$ $(cantidad)$.
      - $PSShrdi_i$ = Productividad del subsistema $i$, en horas **disponibles** $(m^3hrdi^{-1})$, donde:
...
def _(mo):
    ...
    - Sean:
      - $PSS_{1}, PSS_{2}... PSS_{n} =$ Productividad de los Subsistemas $(m^3hrdi^{-1})$.
      - $R =$ Capacidad de recepción en planta $(m^3hrdi^{-1})$.
...
def _(
    ...
    for i in subsistemas_posibles:
        dicc_resultados[i] = [
            ...
            round(cv_subsistema_hrpl(i)),
            round(cv_subsistema_hr_pr_pl(i)),
            round(cmo_subsistema_hrpl(i)),
            round(cf_subsistema_hrpl(i)
                  + cv_subsistema_hrpl(i)
                  + cmo_subsistema_hrpl(i))
        ]
